...
public class Zombie extends AbstractImage{ //Slayer, Search and Destroy, ctf
	...
	private void checkSpawnOutOfBounds()
	{
		...
		if (this.getX() - 5 < 0)
			this.setX(7);
		else if (this.getX() + this.getWidth() + 5 > this.getStage().getWidth())
			this.setX(this.getStage().getWidth() - this.getWidth() - 7);
		//y bounds
		else if (this.getY() - 5 < 0)
			this.setY(7);
		else if (this.getY() + this.getHeight() + 5 > this.getStage().getHeight() - 30)
			this.setY(this.getStage().getHeight() - this.getHeight() - 37);
	}
	
	private void checkOutOfBounds()
	{
		//x bounds
		//Vector2 stageCords = this.getStage().screenToStageCoordinates(new Vector2(body.getX(), body.getY()));
		if (body.getX() < 0)
			setOutOfBounds();
		else if (body.getX() + this.getWidth() > this.getStage().getWidth())
			setOutOfBounds();
		//y bounds
		else if (body.getY() < 0)
			setOutOfBounds();
		else if (body.getY() + this.getHeight() > this.getStage().getHeight() - 30)
			setOutOfBounds();
		else if (outOfBounds)
			outOfBounds = false;
