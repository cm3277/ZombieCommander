...
public class Human extends AbstractImage{
	...
	public void hurt(int damage)
	{
		...
		//health bar
		getStage().addActor(HealthBar.create(getX(),  getY(),  (health)/fullHealth, false));
		getStage().addActor(HealthBar.create(getX(),  getY(),  (health)/fullHealth, true));
		if (health <= 0){
	...
	private void checkOutOfBounds()
	{
		...
		//y bounds
			...
			setOutOfBounds();
		else if (body.getY() + this.getHeight() > 800 - 30)
			setOutOfBounds();
		else if (outOfBounds){
			outOfBounds = false;
			slowReversed = false;
