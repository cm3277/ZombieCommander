...
public class ZombieTarget extends AbstractImage{
	...
	private static boolean setTarget()
    {
    	...
    	{
    		Vector2 touch = getStaticStage().screenToStageCoordinates(new Vector2(Gdx.input.getX(), Gdx.input.getY()));
    		if (!justTouched && !checkButtons((int)touch.x, (int)touch.y)){
    			justTouched = true;
    			touchTimer = 0f;
    			if (quickTargetEnabled){
    				targetArray.clear();
	    		...
	    		//getTargetImage().setPosition((Gdx.input.getX() - getTargetImage().getImageWidth()/2), (800 - Gdx.input.getY())- getTargetImage().getHeight()/2);
	    		getTargetImage().setVisible(false);
	    		Image thisTarget = new Image(getTargetRegion());
	    		thisTarget.setPosition((touch.x - 50/2), (touch.y)- getTargetImage().getHeight()/2);
	...
	public static void loadAssets()
	{
		...
		//if (target == null) {
    		...
    		target.setPosition(480/2 - target.getWidth()/2, 800/2 - target.getHeight()/2);
    	...
    	//}
    		//if (selectText == null) {
        		...
        		selectText.setPosition(480*0.04f, 800*0.95f - selectText.getHeight());
    ...
    private static Label getTargetLabel()
    {
    	if (targetLabel == null) {
    		...
    		targetLabel.setPosition(480/2 - targetLabel.getWidth()/2, (800*0.8f) - targetLabel.getHeight()/2);
