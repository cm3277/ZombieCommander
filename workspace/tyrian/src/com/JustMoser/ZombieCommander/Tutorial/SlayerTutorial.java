...
public class SlayerTutorial extends Actor{
	...
	{
		if (dialogBox == null){
			dialogBox = new DialogBox(getStage().getWidth()/2,getStage().getHeight()/2,"Welcome! As the commander of an elite army of Zombies" +
	...
	private void doTut2()
	{
		if (nextTutStep == false){
			...
			lblInfoHint.setPosition(getStage().getWidth()/2 - lblInfoHint.getWidth()/2, getStage().getHeight()*0.92f);
	...
	private void doTut4()
	{
		if (nextTutStep == false){
			if (AbstractImage.getZombieArray().size > 0){
				dialogBox.setPosition(getStage().getWidth()/2,getStage().getHeight()*0.65f);
